...
export const bookingsRouter = router({
  ...
  book: protectedProcedure
    ...
    .mutation(({ ctx, input }) => bookClass(ctx.db, ctx.user.id, input.classId)),

  cancel: protectedProcedure
    .input(z.object({ bookingId: z.number() }))
    .mutation(({ ctx, input }) =>
      cancelBooking(ctx.db, ctx.user.id, ctx.user.role, input.bookingId),
    ),
  ...
  markAttended: staffProcedure
    ...
    .mutation(({ ctx, input }) => markBookingAttended(ctx.db, input.bookingId, input.source)),
